 ...
 *
 */
public class CSE216Facade extends ProcessingFacadeBase 
{
   ...
   CSE216Facade() 
   {
      ...
      //
      ExecutorService CandS = Executors.newFixedThreadPool(1);

      //
      // Create the client.
      //
      CSE216Client pcm = new CSE216Client(ConfigFile.Instance().getACSIP());
   ...
   synchronized public void processMsg(Object o) 
   {
      try 
      {
         stl.writeToLogger("Server received a message");

      } catch (Exception e) {
         
         stl.writeToLogger( "No request received " + "(telnet test?) ");
         return;
      }
